...
class StringScanner {
		...
		if (idx == len) {
			...
			if (exception == null) {
				if (idx == len) {
					...
					exception = new StringScanException(idx - 1, "unclosed quoted character");
				}
			}
		} else {
			exception = new StringScanException(idx - 1, "uninitialized quoted character");
		}
		if (exception != null) {
			idx = idxSave;
			throw exception;
		...
		if (idx == len) {
			...
			if (idx == len) {
				...
				if (c == '"') {
					...
					exception = new StringScanException(idx - 1, "illegal escape character '" + c + "'");
				}
			}
		} else {
			retval = c;
	...
	public String scanQuotedString() throws StringScanException {
		...
		if (idx == len) {
			...
			exception = new StringScanException(idx - 1, "quoted string must start with \"");
		}
		if (exception != null) {
			idx = idxSave;
			throw exception;
	...
	public String scanNonWhiteSpaceString() throws StringScanException {
		...
		if (idx == len) {
			...
			c = buf[idx++];
			while (idx < len && !Character.isWhitespace(c) && stringDelimiters.indexOf(c) == -1) {
				sbuf.append(c);
				c = buf[idx++];
			}
	...
	public long scanInt(int radix, boolean skipWhite) throws StringScanException {
		...
		if (idx >= len) {
			...
			exception = new StringScanException(idx - 1, "malformed " + baseDesc(radix) + " integer");
		} else {
			val = charval;
			...
			if (Character.isLetter(c = buf[idx]) || Character.isDigit(c) || c == '_') {
				exception = new StringScanException(idx, "malformed " + baseDesc(radix) + " integer");
			}
		}
		if (exception != null) {
			idx = idxSave;
		...
		if (idx == len) {
			...
			if (!hasDigits && (buf[idx] < '0' || buf[idx] > '9')) {
				if (idx == len) {
					...
					exception = new StringScanException(idx, "malformed floating number: no digits");
				}
			} else {
				matchDigits();
...
//	      else
	...
	public boolean scanBoolean() throws StringScanException {
		...
		if (exception == null) {
			if (i < testStr.length() || Character.isLetterOrDigit(c = buf[idx]) || c == '_') {
