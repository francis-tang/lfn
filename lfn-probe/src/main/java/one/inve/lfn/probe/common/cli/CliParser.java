 ...
 * (aliases), <a href=#singleWordOptions>single word options</a>,
 ...
 * <a href=#argsFromAFile>reading arguments from a file</a>. The last feature is
 ...
 *
 * <p>
 * By default, arguments that don't match the specified options, are
 * <a href=#rangespec>out of range</a>, or are otherwise formatted incorrectly,
 ...
 * specifications (although this can be overriden, as
 * <a href=#valueInfo>described below</a>). The application-supplied description
 * is whatever appears in the specification string after the optional
 ...
 * Help options can also be specified by the application using {@link #addOption
 * addOption} and the conversion code <code>%h</code>. Help options can be
 * disabled using {@link #setHelpOptionsEnabled setHelpOptionsEnabled(false)}.
 *
 * <p>
...
//	int tabSpacing = 8;
	 ...
	 * characters which may follow the <code>%</code> character in the specification
	 * string of {@link #addOption addOption}.
	 *
		...
		RangePnt(StringScanner scanner, int type) throws IllegalArgumentException {
			...
			try {
				...
				throw new IllegalArgumentException("Malformed " + typeName + " '"
						+ scanner.substring(scanner.getIndex(), e.getFailIndex() + 1) + "' in range spec");
			}
//	      this.closed = closed;
		}
		...
		public String toString() {
			return "{ dval=" + dval + ", lval=" + lval + ", sval=" + sval + ", bval=" + bval + ", closed=" + closed
					+ "}";
		}
	}
	...
	class Record {
		...
		void scanValue(Object result, String name, String s, int resultIdx) throws CliParseException {
			double dval = 0;
			String sval = null;
			long lval = 0;
			boolean bval = false;
			...
			if (s.length() == 0) {
				throw new CliParseException(name, "requires a contiguous value");
			}
			...
			try {
				...
				throw new CliParseException(name, "malformed " + valTypeName() + " '" + s + "'");
			}
			...
			if (outOfRange) {
				...
				throw new CliParseException(name, "value '" + s + "' not in range " + rangeDesc);
			}
	 ...
	 *
	 * @param synopsisString string that briefly describes program usage, for use by
	 *                       {@link #getHelpMessage getHelpMessage}.
	 * @see CliParser#getSynopsisString
	 * @see CliParser#getHelpMessage
	 */
	public CliParser(String synopsisString) {
		this(synopsisString, true);
	}

	/**
	 * Creates an <code>ArgParser</code> with a synopsis string. The help options
	 * <code>-help</code> and <code>-?</code> are added if <code>defaultHelp</code>
	 * is true.
	 *
	 * @param synopsisString string that briefly describes program usage, for use by
	 *                       {@link #getHelpMessage getHelpMessage}.
	 ...
	 * @see CliParser#getHelpMessage
	 */
	public CliParser(String synopsisString, boolean defaultHelp) {
	 ...
	 * @see CliParser#setSynopsisString
	 * @see CliParser#getHelpMessage
	 */
	 ...
	 * @see CliParser#setHelpOptionsEnabled
	 * @see CliParser#addOption
	 */
	 ...
	 * getHelpMessage} is printed to the default print stream and the program exits
	 * with code 0. Otherwise, arguments which match help options are ignored.
	 *
	 * @param enable enables help options if <code>true</code>.
	 * @see CliParser#getHelpOptionsEnabled
	 * @see CliParser#addOption
	 * @see CliParser#setDefaultPrintStream
	 */
	 ...
	 * @see CliParser#getDefaultPrintStream
	 */
	 ...
	 * @see CliParser#setHelpIndentation
	 * @see CliParser#getHelpMessage
	 */
	...
	/**
	 * Sets the indentation used by {@link #getHelpMessage getHelpMessage}. This is
	 * the number of columns that an option's help information is indented. If the
	 * option's name and value information can fit within this number of columns,
	 * then all information about the option is placed on one line. Otherwise, the
	 * indented help information is placed on a separate line.
	 *
	 * @param indent number of indentation columns
	 * @see CliParser#getHelpIndentation
	 * @see CliParser#getHelpMessage
	 */
...
//  	 }
	...
	/**
	 * Adds a new option description to the parser. The method takes two arguments:
	 * a specification string, and a result holder in which to store the associated
	 * value.
	 *
	 * <p>
	 ...
	 * <ul>
	 * <p>
	 * <li><var>optionNames</var> is a comma-separated list of names for the option
	 * (such as <code>-f, --file</code>).
	 * 
	 * <p>
	 * <li><var>conversionCode</var> is a single letter, following a <code>%</code>
	 ...
	 * <tr>
	 * <td><code>%i</code></td>
	 * <td>an integer, in either decimal, hex (if preceeded by <code>0x</code>), or
	 ...
	 * or <code>\007</code>), and optionally enclosed in single quotes
	 * <tr valign=top>
	 * <td><code>%b</code></td>
	 ...
	 * <tr valign=top>
	 * <td><code>%v</code></td>
	 * <td>no explicit value is expected, but a boolean value of <code>true</code>
	 * (by default) will be stored into the associated result holder if this option
	 * is matched. If one wishes to have a value of <code>false</code> stored
	 * instead, then the <code>%v</code> should be followed by a "range spec"
	 * containing <code>false</code>, as in <code>%v{false}</code>.
	 * </table>
	 *
	 * <p>
	 ...
	 * excluded from the subrange. The values specified in the range spec need to be
	 ...
	 *
	 * <p>
	 * A range spec of <code>{2,4,8,16}</code> for an integer value will allow the
	 ...
	 * 
	 * <p>
	 * A range spec of <code>{(-88,100],1000}</code> for an integer value will allow
	 * values > -88 and <= 100, as well as 1000.
	 *
	 * <p>
	 * A range spec of <code>{"foo", "bar", ["aaa","zzz")} </code> for a string
	 * value will allow strings equal to <code>"foo"</code> or <code>"bar"</code>,
	 * plus any string lexically greater than or equal to <code>"aaa"</code> but
	 ...
	 * value, or a <code>java.util.Vector</code> <a href=#vectorHolder>as discussed
	 * below</a>.
	 *
	 * <p>
	 * <li><var>valueDescription</var> is an optional description of the option's
	 * value requirements, and consists of all characters between two <code>#</code>
	 * characters. The final <code>#</code> character initiates the <i>option
	 * description</i>, which may be empty. The value description is used in
	 * <a href=#helpInfo>generating help messages</a>.
	 *
	 * <p>
	 * <li><var>optionDescription</var> is an optional description of the option
	 * itself, consisting of all characters between a <code>#</code> character and
	 * the end of the specification string. The option description is used in
	 * <a href=#helpInfo>generating help messages</a>.
	 * </ul>
	 *
	 * <p>
	 * The result holder must be an object capable of holding a value compatible
	 * with the conversion code, or it must be a <code>java.util.Vector</code>. When
	 ...
	 * overwritten, unless the result holder is a <code>java.util.Vector</code>, in
	 ...
	 * program, the result holder should be a <code>java.util.Vector</code>.
	 *
	 * <p>
	 * If the result holder is not a <code>Vector</code>, then it must correspond as
	 ...
	 * <tr valign=top>
	 * <td><code>%f</code></td>
	 * <td>{@link argparser.FloatHolder FloatHolder}, {@link argparser.DoubleHolder
	 * DoubleHolder}, <code>float[]</code>, or <code>double[]</code></td>
	 * </tr>
	 * 
	 * <tr valign=top>
	 ...
	 *
	 * <p>
	 * <a name=vectorHolder>If the result holder is a <code>Vector</code>, then the
	 ...
	 *
	 * <p>
	 * The object allocated by the system to store the result will correspond to the
	 * conversion code as follows:
	 * 
	 * <table>
	 * <tr valign=top>
	 * <td><code>%i</code>, <code>%d</code>, <code>%x</code>, <code>%o</code></td>
	 * <td>{@link argparser.LongHolder LongHolder}, or <code>long[]</code> if the
	 * multiplier value exceeds 1</td>
	 * </tr>
	 * 
	 * <tr valign=top>
	 * <td><code>%f</code></td>
	 * <td>{@link argparser.DoubleHolder DoubleHolder}, or <code>double[]</code> if
	 * the multiplier value exceeds 1</td>
	 * </tr>
	 * 
	 * <tr valign=top>
	 * <td><code>%b</code>, <code>%v</code></td>
	 * <td>{@link argparser.BooleanHolder BooleanHolder}, or <code>boolean[]</code>
	 * if the multiplier value exceeds 1</td>
	 * </tr>
	 * 
	 * <tr valign=top>
	 * <td><code>%s</code></td>
	 * <td>{@link argparser.StringHolder StringHolder}, or <code>String[]</code> if
	 * the multiplier value exceeds 1</td>
	 * </tr>
	 * 
	 * <tr valign=top>
	 * <td><code>%c</code></td>
	 * <td>{@link argparser.CharHolder CharHolder}, or <code>char[]</code> if the
	 * multiplier value exceeds 1</td>
	 * </tr>
	 * </table>
	 *
	 ...
	 * @throws IllegalArgumentException if there is an error in the specification or
	 *                                  if the result holder is of an invalid type.
	 */
...
//  	       }
		...
		if (resHolder != null && resHolder.getClass().isArray()) {
			...
			if (rec.numValues > 1 && !(resHolder instanceof Vector)) {
				throw new IllegalArgumentException(
						"Multiplier requires result holder to be an array of length >= " + rec.numValues);
			}
		}
	 ...
	 * Strings are delimited by either whitespace or double quotes <code>"</code>.
	...
	/**
	 * Prints an error message, along with a pointer to help options, if available,
	 * and causes the program to exit with code 1.
	 */
	 ...
	 * matched. The matching starts at a location in <code>args</code> specified by
	 * <code>idx</code>, and unmatched arguments are returned in a String array.
	 *
	 * <p>
	 * In the event of an erroneous argument, the method either prints a message and
	 * exits the program (if {@link #EXIT_ON_ERROR} is set in
	 ...
	 * @param exitFlags conditions causing the program to exit. Should be an or-ed
	 *                  combintion of {@link #EXIT_ON_ERROR} or
	 *                  {@link #EXIT_ON_UNMATCHED}.
	 * @return array of arguments that were not matched, or <code>null</code> if all
	 *         arguments were successfully matched
	 * @see CliParser#getErrorMessage
	 * @see CliParser#getDefaultPrintStream
	 */
	...
	/**
	 * Matches one option starting at a specified location in an argument list. The
	 * method returns the location in the list where the next match should begin.
	 *
	 * <p>
	 ...
	 * unmatched argument. If an argument is matched, {@link #getUnmatchedArgument
	 * getUnmatchedArgument} will return <code>null</code>.
	 *
	 * <p>
	 * If help options are enabled and the argument matches a help option, then the
	 * result of {@link #getHelpMessage getHelpMessage} is printed to the the
	 * default print stream and the program exits with code 0. If help options are
	 ...
	 * @throws CliParseException if there was an error performing the match (such as
	 *                           improper or insufficient values).
	 * @see CliParser#setDefaultPrintStream
	 * @see CliParser#getHelpOptionsEnabled
	 * @see CliParser#getErrorMessage
	 * @see CliParser#getUnmatchedArgument
	 */
	public int matchArg(String[] args, int idx) throws CliParseException {
		unmatchedArg = null;
		setError(null);
		try {
			ObjectHolder ndescHolder = new ObjectHolder();
			...
			if (rec.convertCode == 'h') {
				...
				if (ndesc.oneWord) {
					...
					if (idx + rec.numValues >= args.length) {
						throw new CliParseException(ndesc.name,
								"requires " + rec.numValues + " value" + (rec.numValues > 1 ? "s" : ""));
					}
			...
			if (rec.convertCode != 'v' && rec.convertCode != 'h') {
				if (rec.valueDesc != null) {
					...
					if (rec.rangeDesc != null) {
						optionInfo += "<" + rec.valTypeName() + " " + rec.rangeDesc + ">";
					} else {
